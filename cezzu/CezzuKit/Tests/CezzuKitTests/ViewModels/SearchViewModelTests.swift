...
struct SearchViewModelTests {
    ...
    func loadMoreAppendsNextPage() async {
        let api = HomeViewModelTests.FakeBangumiAPI()
        api.keywordSearchResult = .success((1...30).map { Self.makeItem(id: $0, name: "Item \($0)") })
        let vm = SearchViewModel(api: api)
        vm.text = "机动战士"

        await vm.submit()

        api.keywordSearchResult = .success((31...34).map { Self.makeItem(id: $0, name: "Item \($0)") })
        await vm.loadMoreIfNeeded(currentItem: vm.results[29])

        #expect(vm.results.count == 34)
        #expect(vm.results.last?.id == 34)
        #expect(vm.hasMore == false)
        #expect(api.keywordSearchCalls.count == 2)
        #expect(api.keywordSearchCalls[1].2 == 30)
    }
}
